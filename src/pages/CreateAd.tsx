...
export default function CreateAd() {
  ...
  const [city, setCity] = useState(LUANDA_MUNICIPALITIES[4]); // Luanda as default
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!title || !description || !phone || !companyName) {
    ...
    adService.createAd({
      ...
      status: 'Aprovado', // Admins publish instantly!
      advertiser: user?.user_metadata?.full_name || 'Administrador',
      advertiser_email: user?.email || 'admin@bazarapido.com',
  ...
  if (loading || !user || user.email !== 'frankmanuel123.com@gmail.com') {
    return (
      ...
      <div className="max-w-md mx-auto px-4 space-y-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs font-medium text-gray-500">Publicação imediata como administrador</p>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="bg-white p-6 rounded-3xl border border-gray-100/80 shadow-xl shadow-gray-200/40 space-y-5">
          ...
          <div className="space-y-1">
            ...
            <input 
              type="text"
              required
              placeholder="Ex: Aluguer Executivo Ford Ranger"
              value={title}
              onChange={(e) => setTitle(e.target.value)}
              className="w-full px-4 py-3 bg-gray-50 border border-gray-100 rounded-2xl text-sm font-semibold text-gray-800 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-amber-500/20 focus:border-amber-500 transition-all"
            />
          </div>
          ...
          <div className="space-y-1">
            ...
            <div className="relative">
              <input 
                type="text"
                required
                placeholder="Ex: Luanda Rents Lda"
                value={companyName}
                onChange={(e) => setCompanyName(e.target.value)}
                className="w-full pl-10 pr-4 py-3 bg-gray-50 border border-gray-100 rounded-2xl text-sm font-semibold text-gray-800 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-amber-500/20 focus:border-amber-500 transition-all"
              />
          ...
          <div className="space-y-1">
            ...
            <textarea 
              rows={4}
              required
              placeholder="Indique todas as informações do serviço, viatura, rotas ou horários disponíveis."
              value={description}
              onChange={(e) => setDescription(e.target.value)}
              className="w-full p-4 bg-gray-50 border border-gray-100 rounded-2xl text-sm font-semibold text-gray-800 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-amber-500/20 focus:border-amber-500 transition-all"
            />
          </div>
